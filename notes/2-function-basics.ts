...
function getNameParts(contact: { name: string }) {
  ...
  if (parts.length === 1) { // add this case for when you have a celeb like Madonna or Prince
                            // but this changes the properties to have possible
                            // undefined types
    return { name: parts[0]}
  }
  if (parts.length < 2) {
...
// 🚨 mixing does not work // because we added the overload signatures above
